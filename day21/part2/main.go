...
func main() {
	...
	output := BuildCanonicalDangerousIngredientList(mapping)
...
func BuildCanonicalDangerousIngredientList(mapping map[string]string) string {
	...
	for i := 0; i < len(keys)-1; i++ {
		builder.WriteString(mapping[keys[i]])
		builder.WriteRune(',')
	}
	builder.WriteString(mapping[keys[len(keys)-1]])
